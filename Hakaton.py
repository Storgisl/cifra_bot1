...
async def create_command(message: types.Message):
    await message.answer('Давай же начнем! Для начала выбери свой отдел', reply_markup=kb1())
...
async def load_department(message: types.Message, state: FSMContext):
    ...
    await message.answer('Теперь укажи свой пол', reply_markup=kb2())
...
async def load_age(message: types.Message, state: FSMContext):
    ...
    await message.answer('Теперь укажи своё хобби', reply_markup=kb3())
    await QuestionnaireStatesGroup.next
...
async def load_hobbies(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(chat_id=message.from_user.id, caption = f"{data['department']},{data['sex']},{data['age']},{data['hobbies']}")
