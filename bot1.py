...
async def db_start():
    ...
    db.commit()
def show_profiles():
    user = sq.connect("Users.db").cursor().execute(f"SELECT * FROM profile  ORDER BY hobbies DESC").fetchall()
    sq.connect("Users.db").commit()
    return user
...
async def edit_profile(state, user_id):
    async with state.proxy() as data:
        cur.execute("UPDATE profile SET department = '{}', sex = '{}', age = '{}', hobbies = '{}'  "
                    "WHERE user_id == '{}'".format(data['department'], data['sex'], data['age'], data['hobbies'], user_id))
        db.commit()
...
async def start_command(message: types.Message) -> None:
    ...
    await create_profile(user_id=message.chat.username)
...
async def load_hobbies(message: types.Message, state: FSMContext) -> None:
    ...
    await edit_profile(state, user_id=message.chat.username)
    await state.finish()
    await bot.send_message(chat_id=message.from_user.id, text=f"{my_str}")
